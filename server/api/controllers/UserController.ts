...
export class UserController extends Controller {
  ...
  public async register(@inversify.request() req: Request) {
    if (
      !this.validateService.requestBody(
    ...
    if (await this.authService.userExists('email', email)) {
      return this.json(
        { error: { email: this.ErrorMessages.EMAIL_TAKEN } },
        422
      )
    }
    ...
    if (!user)
      throw new Error(
        `Failed creating user with email ${email} upon registration.`
      )
  ...
  public async login(
    ...
    if (!user) {
      return this.json(
        { error: { email: this.ErrorMessages.USER_NOT_EXISTS } },
        422
      )
    }
